# Shared Docker Development Rules

This guide records the shared-machine Docker rules for SGLang Omni, Miles, and
Slime development.
...
- With this mount, the host directory `/data/<your-name>` appears as `/data`
  ...
  `/data`, for example `/data/sglang-omni` and `/data/runs`. Do not rely on
  files left only in `/root`, `/workspace`, `/tmp`, or the container image
  layer.
- Put virtual environments under `/data` as well, for example `/data/.venv`.
  That keeps the Python environment reusable if the container is stopped or
  re-entered.
...
- Code and experiment outputs are stored under the mounted `/data` path.
