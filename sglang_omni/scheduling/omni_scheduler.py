...
class OmniScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        # engages, so a lone request never pays the idle
        # wait (avoids the low-concurrency latency tax
        # measured in the issue #760 audio A/B).
    ...
    def _coalesce_prefill_window(self) -> None:
        ...
        # one iteration in result_queue (running_batch
        # still 0) before its decode starts; don't stall
        # that transition. Attribute only exists in
        # _event_loop_overlap.
        ...
        # its transient activation/broadcast memory) to one
        # chunk-worth; prevents the multimodal thinker-TP
        # OOM seen at high concurrency (issue #760).
