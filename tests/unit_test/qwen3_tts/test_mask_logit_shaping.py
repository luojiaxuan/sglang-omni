...
def test_qwen3_tts_masks_silence_ids_only_in_opening_frames_of_flagged_requests() -> None:
    configured_vocab = 3072
    silence_ids = (5, 7, 11)
    runner = _runner(
        vocab_size=configured_vocab,
    ...
    speech = [token for token in range(configured_vocab - 1024) if token not in silence_ids]
